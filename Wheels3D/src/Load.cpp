...
GLuint Load::Text(const char * str, int r, int g, int b) {
	...
	TTF_Font *font = TTF_OpenFont("model/DS-DIGIB.ttf", 24);
	if (font == NULL)
	...
	if (stringImage == NULL) {
		...
		std::cout << "String surface not created when trying to create image from text: " << str << "\n";
		return -1;
	}
	else {
...
GLuint Load::Text(const char * str, bool withDigitsFont) {
	if (TTF_Init() == -1)
		std::cerr << "TTF failed to initialise.\n";
	TTF_Font *font;
	if (withDigitsFont)
		font = TTF_OpenFont("model/DS-DIGIB.ttf", 24);
	else
		font = TTF_OpenFont(".\\model\\MavenPro-Regular.ttf", 24);
	if (font == NULL)
		std::cerr << "Failed to open font.\n";
...
GLuint Load::CubeMap(const char *fname[6], GLuint *texID)
{
	...
	for (int i = 0; i<6; i++)
	{
		...
		if (!tmpSurface)
		{
			std::cout << "Error loading bitmap " << fname[i] << " when trying to Load::CubeMap \n";
			return *texID;
		}
...
GLuint Load::BMP(char *fname) {
	...
	if (!tmpSurface) {
		std::cout << "Error loading bitmap " << fname << " when trying to Load::BMP \n";
		return -1;
	}
	else
	{
...
GLuint Load::PNG(char *fname) {
	...
	if (!(IMG_Init(imgFlags) & imgFlags))
	{
		...
		if (!tmpSurface) {
			std::cout << "Error loading image file " << fname << " when trying to Load::PNG function \n";
			return -1;
		}
		else {
			// bind texture and set parameters
			...
			SDL_FreeSurface(tmpSurface); // texture loaded, free the temporary buffer
			if (texID != 0) std::cout << "Loaded image " << fname << "\n";
			return texID;	// return value of texture ID
		}
	}
}
