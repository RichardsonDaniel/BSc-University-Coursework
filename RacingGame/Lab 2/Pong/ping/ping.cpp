...
void draw() {
	...
	// draw speed
    drawText(width / 2 - 10, height - 15, "Speed:" + int2str(ball_speed));
	// draw ball
...
void keyboard() {
	...
	if (GetAsyncKeyState(VK_SPACE)) ball_speed+=0.5;

	//reset speed
	if (GetAsyncKeyState(VK_R)) ball_speed=2;
}
...
void update(int value) {
   keyboard();
   updateBall();
   ball_speed+=0.005; //increase acceleration
