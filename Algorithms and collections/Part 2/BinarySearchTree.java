...
/** Part B - intended and actual changes within.
 * 
 ...
 * 
 * Changes Intended:
 *  - Modify your solution to Part A to provide a constructor for the BinarySearchTree<E>
 *    class that takes a boolean parameter which, if true, means that the add() method
 *    inserts at the root and if false will insert in an external node. Do not provide
 *    any method to change the insertion method once a tree has been created. This means
 *    that when a BinarySearchTree is created it will either always insert at the root
 *    or always in a leaf. You may also retain the default constructor which takes
 *    no parameter – you should ensure that by default the tree will insert using
 *    the standard algorithm (not at the root). Note that, whether insertion is
 *    at the root or in an external (leaf) node, the tree should still be an
 *    extended binary search tree that does not allow duplicate values.
 *  
 * Changes Performed:
 * 
 *  - Added addRoot to the initialization section of BinarySearchTree and then initialized
 *    it within the BinarySearchTree and set it to false by default as specified.
 *    
 *  - Created a second BinarySearchTree constructor to change between adding at root
 *    or leaf upon construction so it cannot be changed later as specified.
 *    
 *  - Created a second add method to decide which insertion action to perform and watch
 *    for duplicates early on to keep them both extended binary search trees.
 *  
 *  - Changed original add method to now be addLeaf nothing fundamentally has changed
 *    it is now just used when needing to add a leaf.
 *    
 *  - Created a rotateLeft method based off of the rotate left function on page 433
 *    of william j collins book and the notes provided, used to rearrange tree for
 *    root additions.
 *  
 *  - Created a rotateRight method which is based of of the rotate left method created earlier
 *    however values are switched to allow a right rotation.
 *  
 *  - Added a method so that new elements can be added to the root, it does this by monitoring
 *    if elements inserted are higher or lower than the current elements and rotating the tree appropriately 
 *    so that insert at the root will work.
 *  
 **/
